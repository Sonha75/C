                                   ...
                                   uint8_t buff[2])
{
  ...
  emberAfSetCommandEndpoints(SWITCH_ENDPOINT, 3);
...
static void binding_table_print(void)
{
 ...
 void attribute_report_event_handler(sl_zigbee_event_t *event)
{
  float height=sensor_data.distance;
  float weight=sensor_data.pressure;
  printf("\nheight:%f\n",height);
  printf("\nweight:%f\n",weight);
  EmberStatus status = EMBER_SUCCESS;
  union {
  ...
  if (emberAfNetworkState() != EMBER_JOINED_NETWORK) {
    ...
    attribute.can_nang =weight / 1000;



    // Send report directly
    status = reportAttribute( ZCL_WEIGHT_BIN_ID,
                              ZCL_WEIGHT_MEASURED_VALUE_ATTRIBUTE_ID,
                              ZCL_INT16S_ATTRIBUTE_TYPE,
                              (uint8_t *)&attribute.can_nang);

    // Just update attribute data to send report to bindings
    status = emberAfWriteServerAttribute(SWITCH_ENDPOINT,
                                         ZCL_WEIGHT_BIN_ID,
                                         ZCL_WEIGHT_MEASURED_VALUE_ATTRIBUTE_ID,
                                         (uint8_t *)&attribute.can_nang,
                                         ZCL_INT16S_ATTRIBUTE_TYPE);
    ...
    attribute.chieu_cao =height / 1000;

    // Send report directly
    status = reportAttribute( ZCL_HEIGHT_BIN_ID,
                              ZCL_HEIGHT_MEASURED_VALUE_ATTRIBUTE_ID,
                              ZCL_INT16S_ATTRIBUTE_TYPE,
                              (uint8_t *)&attribute.chieu_cao);
...
void emberAfPluginFindAndBindInitiatorCompleteCallback(EmberStatus status)
{
  sl_zigbee_app_debug_println("Find and bind initiator %s: 0x%02X", "complete", status);
   if (status == EMBER_SUCCESS) {
       //sl_zigbee_event_set_delay_ms(&attribute_report_event_control,9000);
     } else {
