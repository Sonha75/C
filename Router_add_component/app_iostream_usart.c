...
void USART0_RX_IRQHandler(void)
{
  ...
  if ((buffer[inpos] != '&') && (inpos < BUFLEN))
    inpos++;
      ...
      float array_1d[7];
      for(int i = 0; i < 7; i++) {
             array_1d[i] = atoi((char*)chuoi[i]);
         }

      sensor_data.id=1;
      sensor_data.temperature=array_1d[1];
      sensor_data.humidity=array_1d[2];
      sensor_data.mass=array_1d[3];
      sensor_data.height=array_1d[4];
      sensor_data.smell=array_1d[5];
      sensor_data.battery=array_1d[6];
      memcpy(sensor_data.node_name,"node1",6);
