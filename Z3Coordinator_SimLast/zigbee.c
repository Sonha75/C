...
static void parse_received_data(EmberMessageBuffer message, sensor_data_t *parsed_data) {
  ...
  emberCopyFromLinkedBuffers(message,
     ...
     if(mang[0] == 00){
       memcpy(&parsed_data->id,mang+1, messageLen-1);
         ...
         memcpy(&parsed_data->mass,mang+1, messageLen-1);

     }else if(mang[0] == 04){
         memcpy(&parsed_data->height,mang+1, messageLen-1);

     }
     else if(mang[0] == 05){
         memcpy(&parsed_data->smell,mang+1, messageLen-1);
     }
     else if(mang[0] == 06){
         memcpy(&parsed_data->battery,mang+1, messageLen-1);
