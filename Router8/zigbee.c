...
void copy_to_messaget(uint8_t **message_uni, sensor_data_t *sensor_data) {
    ...
    *(*(message_uni+6)+0)=06;
    ...
    // Copy distance
    *(*(message_uni+7)+0)=07;
    memcpy( *(message_uni+7)+1, &sensor_data->index, sizeof(sensor_data->index));
}
...
//send by row
 for(int i=0;i<8;i++)
  {
    emberAfSendUnicast(EMBER_OUTGOING_DIRECT, 0x0000, &apsFrame, kich_thuoc[i], *(message_uni + i) ); //gia tri con tro chi den o nho dau tien->dia chi o nho dau tien
  }
...
{
  sl_zigbee_event_init(&commissioning_event, commissioning_event_handler); //ko qtam
  sl_zigbee_event_init(&led_event, led_event_handler); //ko qtam
  sl_zigbee_event_init(&finding_and_binding_event, finding_and_binding_event_handler); // tu dong
  sl_zigbee_event_init(&send_message_event, send_message_test); // core
  //sl_zigbee_event_init(&send_message_event1, send_multicast_message);
// setup multicast entry
