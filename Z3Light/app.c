...
#define RHT_ENDPOINT           3

static sl_zigbee_event_t commissioning_led_event;
static sl_zigbee_event_t finding_and_binding_event;
static sl_zigbee_event_t read_attribute_event;
//---------------

//happen when receive a reported attribute
                                     ...
                                     int16u bufLen)
{
      ...
      clusterId == ZCL_TEMP_MEASUREMENT_CLUSTER_ID)
  {
      ...
      if (clusterId == ZCL_RELATIVE_HUMIDITY_MEASUREMENT_CLUSTER_ID){
          emberAfCorePrintln("\nRH: %2d.%2d %%\n",attribute/100, (attribute % 100));
      }else{
          emberAfCorePrintln("\nTemperature: %2d.%2d C\n",attribute/100, (attribute % 100));
      }
      return true;
  }
  if(clusterId == ZCL_HEIGHT_BIN_ID ||
       clusterId == ZCL_WEIGHT_BIN_ID)
   {
       ...
       if (clusterId == ZCL_HEIGHT_BIN_ID){
           ...
           emberAfCorePrintln("\Weight: %2d.%2d \n",attribute/100, (attribute % 100));
       }
       return true;
   }
...
//// Event handlers
static void read_attribute_event_handler(sl_zigbee_event_t *event)
{
  float tmp;
  float humi;
  if (emberAfNetworkState() == EMBER_JOINED_NETWORK)
    {
      emberAfReadServerAttribute(RHT_ENDPOINT,ZCL_TEMP_MEASUREMENT_CLUSTER_ID,ZCL_TEMP_MEASURED_VALUE_ATTRIBUTE_ID,(uint8_t*)&tmp,sizeof(tmp));
      emberAfReadServerAttribute(RHT_ENDPOINT,ZCL_RELATIVE_HUMIDITY_MEASUREMENT_CLUSTER_ID,ZCL_RELATIVE_HUMIDITY_MEASURED_VALUE_ATTRIBUTE_ID,(uint8_t*)&humi,sizeof(humi));
  ...
  sl_zigbee_event_set_delay_ms(&read_attribute_event,10000);
}
...
static void finding_and_binding_event_handler(sl_zigbee_event_t *event)
{
  if (emberAfNetworkState() == EMBER_JOINED_NETWORK) {
    ...
    sl_zigbee_app_debug_println("Find and bind target start: 0x%02X",
                                emberAfPluginFindAndBindTargetStart(RHT_ENDPOINT));
...
void emberAfMainInitCallback(void)
{
  ...
  sl_zigbee_event_init(&read_attribute_event, read_attribute_event_handler);
